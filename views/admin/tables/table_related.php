...
$aColumns = [
    ...
    db_prefix() . 'licence_items.task_id',
...
$join = [
    ...
    'JOIN ' . db_prefix() . 'inspection_items ON ' . db_prefix() . 'inspection_items.task_id = ' . db_prefix() . 'tasks.id',
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == db_prefix() . 'tasks.name') {
        ...
        }elseif ($aColumns[$i] == db_prefix() . 'licence_items.task_id') {
            ...
            $_data = '';
            if($aRow[db_prefix() . 'inspection_items.equipment_name'] !== NULL){
