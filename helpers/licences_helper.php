...
hooks()->add_action('app_admin_head', 'licences_head_component');
//hooks()->add_action('app_admin_footer', 'licences_footer_js__component');
 ...
 * @return array
 */
function get_licences_percent_by_status($status, $project_id = null)
{
 ...
 * @return null
 */
function licences_head_component()
{
    $CI = &get_instance();
    if (($CI->uri->segment(1) == 'admin' && $CI->uri->segment(2) == 'licences') ||
        $CI->uri->segment(1) == 'licences'){
        echo '<link href="' . base_url('modules/licences/assets/css/licences.css') . '"  rel="stylesheet" type="text/css" >';
    }
}
 ...
 * @return null
 */
function licence_head_component()
{
}

$CI = &get_instance();
// Check if licence is excecuted
if ($CI->uri->segment(1)=='licences') {
    hooks()->add_action('app_customers_head', 'licence_app_client_includes');
}
 ...
 * @return stylesheet
 */
function licence_app_client_includes()
{
    echo '<link href="' . base_url('modules/' .LICENCES_MODULE_NAME. '/assets/css/licences.css') . '"  rel="stylesheet" type="text/css" >';
    echo '<script src="' . module_dir_url('' .LICENCES_MODULE_NAME. '', 'assets/js/licences.js') . '"></script>';
}


function after_licence_updated($id){
...
function licence_status_changed_hook($data){
